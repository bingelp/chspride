        ...
        <div className="container container-full">
          ...
          <div className="collapse navbar-collapse" id="ms-navbar">
            <ul className="navbar-nav">
              {this.props.left.map(e => <NavDropdown key={e.key} title={e.title} pages={e.pages} />)}
            </ul>
            <ul className="navbar-nav ml-auto">
              {this.props.right.map(e => <NavItem key={e.key} title={e.title} background={e.background} to={e.to} icon={e.icon} />)}
            </ul>
          </div>
