...
export default class SponsorHero extends React.Component {
    ...
    componentDidMount() {
        fetch("https://3xy42f3mwb.execute-api.us-east-1.amazonaws.com/dev/sponsors")
        .then(results => {
            return results.json();
        })
        .then(data => {
    ...
    render() {
        return (
            <div className="container text-center">
                ...
                <div className="row">
                    {this.state.sponsors && this.state.sponsors.slice(0, 6).map(s => <Sponsor key={s.id} meta={s} />)}
                </div>
